...
let hitokotos = [{"hitokoto":"若隐若现才是艺术！"}]
        ...
        switch (otext[0]) {
            case 'start':
                await tg(token,'sendmessage',{
                    chat_id: chat_id,
                    text: '欢迎使用，您可以使用 /hitokoto 来输出随机一段一言，或者在其他群里面 @hitokoto_bot 来快速寻找'
                })
                break
            case 'hitokoto':
                let h = hitokoto()
                await tg(token,'sendmessage',{
                    chat_id: chat_id,
                    text: h.hitokoto // + h.source == '' ? '' : ' -- ' + h.source
                })
                break
        }
    }
...
async function handleinline(d){
    ...
    let hh = shuffle(hitokotos,offset[0]) // 随机排序
    if(query !== '')
        hh = hh.filter(h=>{
